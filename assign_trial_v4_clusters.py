        ...
        with codecs.open(file_trials, "rb", "utf-8") as f_trials:
            ...
            for person in people:
                ...
                for work in trials:
                    ...
                    output_filename = "results/%s_%s_results_%s_%.2f.csv" % (person.lower(), results_prefix, similarity.__name__, T)
                    df.to_csv(output_filename, index=False)

                    print("  *", "written {} results [{}]".format(len(df), output_filename))
                    ...
                    print(
                        "[T={}], [sim={}], hamming score (label-based accuracy): {}".format(
                            T, similarity.__name__, hamming_score(y_true, y_pred)
                        )
                    )
